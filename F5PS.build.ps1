...
task Test {
    try {
        ...
        Invoke-ScriptAnalyzer ".\F5PS\Public" -Recurse
        Write-Verbose -Message "Running PSScriptAnalyzer on Private functions"
        Invoke-ScriptAnalyzer ".\F5PS\Private" -Recurse
    }
    catch {
...
Task FixTrailingWhitespaces {
	$files = Get-ChildItem ".\F5PS" -include *.ps1 -Recurse
...
task DebugBuild -if ($Configuration -eq "debug") {
    $Script:ModuleName = (Test-ModuleManifest -Path ".\F5PS\*.psd1").Name
    ...
    if(!($ModuleVersion)) {
        ...
        $ModuleVersion = (Test-ModuleManifest -Path ".\F5PS\$($ModuleName).psd1").Version
    ...
    try {
        Copy-Item -Path ".\F5PS\$($ModuleName).psd1" -Destination ".\Output\temp\$($ModuleName)\$ModuleVersion\"
        #$ExistingPSM1 = ".\F5PS\$($ModuleName).psm1"
        ...
        throw "Failed copying Module Manifest from: .\F5PS\$($ModuleName).psd1 to .\Output\temp\$($ModuleName)\$ModuleVersion\ or Generating the new psm file."
    }

    Write-Verbose -Message "Updating Module Manifest with Public Functions"
    $publicFunctions = Get-ChildItem -Path ".\F5PS\Public\*.ps1"
    $privateFunctions = Get-ChildItem -Path ".\F5PS\Private\*.ps1"
    try {
    ...
    foreach($function in $publicFunctions.Name){
        try {
            ...
            $content = Get-Content -Path ".\F5PS\Public\$($function)"
            Add-Content -Path $ModuleFile -Value "#Region - $function"
            Add-Content -Path $ModuleFile -Value $content
            if($ExportAlias.IsPresent){
                $AliasSwitch = $false
                $Sel = Select-String -Path ".\F5PS\Public\$($function)" -Pattern "CmdletBinding" -Context 0,1
    ...
    foreach($function in $privateFunctions.Name){
        try {
            Write-Verbose -Message "Updating the .psm1 file with function: $($function)"
            $content = Get-Content -Path ".\F5PS\Private\$($function)"
            Add-Content -Path $ModuleFile -Value "#Region - $function"
            Add-Content -Path $ModuleFile -Value $content
            Add-Content -Path $ModuleFile -Value "#EndRegion - $function"            
        }
...
task Build -if($Configuration -eq "Release"){
    ...
    if(!($ModuleVersion)) {
        Write-Verbose -Message "No new ModuleVersion was provided, locating existing version from psd file."
        $oldModuleVersion = (Test-ModuleManifest -Path ".\F5PS\$($ModuleName).psd1").Version
        ...
        #
        # Update-ModuleManifest -Path ".\F5PS\$($ModuleName).psd1" -ModuleVersion $ModuleVersion
    }
    ...
    Write-Verbose -Message "Generating the Module Manifest for temp build and generating new Module File"
    try {
        Copy-Item -Path ".\F5PS\$($ModuleName).psd1" -Destination ".\Output\$($ModuleName)\$ModuleVersion\"
        New-Item -Path ".\Output\$($ModuleName)\$ModuleVersion\$($ModuleName).psm1" -ItemType File
    }
    catch {
        throw "Failed copying Module Manifest from: .\F5PS\$($ModuleName).psd1 to .\Output\$($ModuleName)\$ModuleVersion\ or Generating the new psm file."
    }
